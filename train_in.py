...
def create_model(labelBinarizer):
    # initializing
    model = Sequential()
    ...
    model.add(Dense(len(labelBinarizer.classes_), activation="softmax", name="out"))

    return model

def update_model(model, labelBinarizer):
    
    previous_weights = model.get_layer("out").get_weights()
    # remove last layer
    model.pop()
    # add new output_layer
    predict_layer = Dense(len(labelBinarizer.classes_), activation='softmax', name="out")
...
def reduce_class_data(trainX, trainY, current_label, labels, model, data_size=0.2, select_mode='best'):
    ...
    if select_mode =='random':
    ...
    elif select_mode =='bad':
        # sort by bad
        selected_data = sorted(x_y, key=lambda l: l[1][current_label], reverse=False)
        ...
        # sort by best
        selected_data = sorted(x_y, key=lambda l: l[1][current_label], reverse=True)
        selected_data = np.asarray(selected_data)
        # choose first data_size samples
        selected_data = selected_data[0:data_size, 0]

    print("[Info] from "+labels[current_label]+" select "+str(data_size)+"/"+str(len(x_y))+" for incremental learning...")
    data, labs = [], []
...
for folder, i in zip(folders, range(num_labels)):
    print('[Info] loading images from "' + folder + '".........')
    images_path = sorted(list(paths.list_images(base_path + "/" + folder)))
    ...
    if i == 1:
        ...
        data = np.array(data_list, dtype="float") / 255.0
        ...
        previous_data_1, previous_labels_1 = reduce_class_data(trainX=trainX, trainY=trainY, current_label=0,
                                                               labels=folders, data_size=0.2, model=model)
    ...
    if i >= 2:
            print('[Info] training network for '+str(i+1)+' classes....')
            data = np.array(data_list, dtype="float") / 255.0
            labels = np.array(labels_list)
            ...
            testX = np.array(previous_trainX)
            trainY = lb.fit_transform(trainY)
            ...
            print(rp)
            if i < num_labels-1:
                previous_data, previous_labels = reduce_class_data(trainX=trainX, trainY=trainY, current_label=i,
                                                                   labels=folders, data_size=0.2, model=model)
